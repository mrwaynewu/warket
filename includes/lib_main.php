...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = $GLOBALS['_CFG']['shop_title'] . ' - ' . 'Powered by PBCC';
...
function assign_template($ctype = '', $catlist = array())
{
	...
	//show_array($navigator);
	$temp_bottom = array();
	foreach($navigator['bottom'] as $key => $value){
		if ($value['name'] == '新手上路'){
			$temp_bottom[$key] = $value;
			$temp_bottom[$key]['bg_img_src'] = 'xin_shou_shang_lu.png';
		}
		else if($value['name'] == '购物指南'){
			$temp_bottom[$key] = $value;
			$temp_bottom[$key]['bg_img_src'] = 'gou_wu_zhi_nan.png';
		}
		else if($value['name'] == '支付配送'){
			$temp_bottom[$key] = $value;
			$temp_bottom[$key]['bg_img_src'] = 'zhi_fu_pei_song.png';
		}
		else if($value['name'] == '售后服务'){
			$temp_bottom[$key] = $value;
			$temp_bottom[$key]['bg_img_src'] = 'shou_hou_fu_wu.png';
		}
		else if($value['name'] == '关于微市'){
			$temp_bottom[$key] = $value;
			$temp_bottom[$key]['bg_img_src'] = 'guan_yu_wei_shi.png';
		}
	}
	
	
	$navigator['bottom'] = $temp_bottom;
	$temp_bottom = array();
	/// assign childern to bottom nav ////
	$temp_navigator_children = array();
	foreach ($navigator['bottom'] as $key => $value){
		$temp_navigator_children[] = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('article') .'where cat_id = '.$value['cid'].' and is_open = 1');
	}
	foreach ($temp_navigator_children as $key => $value){
		foreach($value as $vkey => $vvalue){
			$value[$vkey]['url'] = 'article.php?id='.$vvalue['article_id'].'';
		}
		$navigator['bottom'][$key]['children'] = $value;
	}

    $smarty->assign('navigator_list',$navigator);  //自定义导航栏
...
{	
    ...
    $cat_id = $GLOBALS['db']->GetAll('SELECT cat_id FROM ' . $GLOBALS['ecs']->table('article') . "where article_id = '$article_id'");
...
function get_article_parent_id($cat_id)
{	
	$parent_id = '';
    if ($cat_id == 0)
    {
    ...
    $parent_id = $GLOBALS['db']->GetAll('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat') . "where cat_id = '$cat_id'");
...
function get_subcat_info($cat_id, $show_in_nav='')
{	
	if ($show_in_nav == ''){$show_in_nav = false;}
	else {$show_in_nav = true;}
	...
	if ($show_in_nav === true){
		$cat_info = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('article_cat') . "where parent_id = '$cat_id' and show_in_nav = 1");
	}
	else {
		$cat_info = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('article_cat') . "where parent_id = '$cat_id'");
	}
...
/**
 * 获得指定articles based on article_cat
 *
 * @access  public
 * @param   integer $cat    分类编号
 * @return  array
...
function get_article_info_from_article_cat($cat_arry)
{	
    if (empty($cat_arry))
    {
        return array();
	...
	$article_info = array();
	foreach ($cat_arry as $key => $value){
		$cat_id = $value['cat_id'];
		$article_info[] = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('article') . "where cat_id = '$cat_id' and is_open = 1");
	}
...
function get_left_article_nav($id,$type,$show_in_nav=""){//$type: 0 means its based on article_id, 1 means its based on cat_id
	if ($id == ''){return array();}
	$article_id = '';
	if ($type == 0){
		$cat_id = get_article_cat_id($id);
		$article_id = $id;
	}
	else {
		$cat_id = $id;
	...
	$parent_id = get_article_parent_id($cat_id);

	$parent_id = 3;
	$cat_arry = get_subcat_info($parent_id,$show_in_nav);// true: show_in_nav == 1;
	$article_arry = get_article_info_from_article_cat($cat_arry);

	$temp_article_arry = array();
	foreach ($article_arry as $key => $value){
		foreach($value as $subvalue){$temp_article_arry[] = $subvalue;}
	}
	$article_arry = $temp_article_arry;$temp_article_arry = array();
	//show_array($article_arry);

	$temp_article_cat_with_child = array();
	foreach ($cat_arry as $cat_key => $cat_value){
		$temp_article_cat_with_child[$cat_value['cat_id']] = array();
		$temp_article_cat_with_child[$cat_value['cat_id']] = $cat_value;
		$temp_article_cat_with_child[$cat_value['cat_id']]['url'] = 'article_cat.php?id='.$cat_value['cat_id'].'';
		$temp_article_cat_with_child[$cat_value['cat_id']]['highlight'] = '0';
		if ($cat_id == $cat_value['cat_id']){
			$temp_article_cat_with_child[$cat_value['cat_id']]['hightlight'] = "1";
		}
		foreach ($article_arry as $article_key => $article_value){
			if ($cat_value['cat_id'] == $article_value['cat_id']){
				$article_value['hightlight'] = "0";
				
				if ($article_id == $article_value['article_id']){////hight article id////
					$article_value['hightlight'] = "1";
					}
					
				$article_value['url'] = 'article.php?id='.$article_value['article_id'].'';
				$temp_article_cat_with_child[$cat_value['cat_id']]['children'][] = $article_value;
			}
		}
	}
	$cat_arry = $temp_article_cat_with_child;$temp_article_cat_with_child = array();
...
function get_goods_cat_id($goods_id)
{	
	$cat_id = '';
    if ($goods_id == 0)
    {
    ...
    $cat_id = $GLOBALS['db']->GetAll('SELECT cat_id FROM ' . $GLOBALS['ecs']->table('goods') . "where goods_id = '$goods_id'");
...
function get_goods_parent_id($cat_id)
{	
	$parent_id = '';
    if ($cat_id == 0)
    {
    ...
    $parent_id = $GLOBALS['db']->GetAll('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . "where cat_id = '$cat_id'");
...
function get_all_cat_of_goods_info($show_in_nav='')
{	
	if ($show_in_nav == ''){$show_in_nav = false;}
	else {$show_in_nav = true;}
	...
	if ($show_in_nav === true){
		$cat_info = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('category') . "where is_show = 1 order by sort_order");
	}
	else {
		$cat_info = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('category') . "order by sort_order");
	}
...
function get_goods_info_from_goods_cat($cat_arry)
{	
    if (empty($cat_arry))
    {
        return array();
	...
	foreach ($cat_arry as $key => $value){
		$cat_id = $value['cat_id'];
		$goods_info[] = $GLOBALS['db']->GetAll('SELECT * FROM ' . $GLOBALS['ecs']->table('goods') . "where cat_id = '$cat_id'");
	}
...
function get_left_goods_nav($id,$type,$show_in_nav=""){//$type: 0 means its based on goods_id, 1 means its based on cat_id

	if ($id == ''){return array();}
	$goods_id = '';
	if ($type == 0){
		$cat_id_of_goods = get_goods_cat_id($id);
		$cat_id = get_goods_parent_id($cat_id);
		$goods_id = $id;
	}
	else {
		$cat_id = $id;
	...
	$cat_arry = get_all_cat_of_goods_info($show_in_nav);// true: show_in_nav == 1;
	
	$top_cat_arry = array();
	$nontop_cat_arry = array();
	foreach ($cat_arry as $key => $value){
		if ($value['parent_id'] == 0){
			$top_cat_arry[] = $value;
		}
		else {
			$nontop_cat_arry[] = $value;
			}
	}

	$temp_cat_with_child = array();
	foreach ($top_cat_arry as $top_cat_key => $top_cat_value){
		$temp_cat_with_child[$top_cat_value['cat_id']] = array();
		$temp_cat_with_child[$top_cat_value['cat_id']] = $top_cat_value;
		$temp_cat_with_child[$top_cat_value['cat_id']]['url'] = 'category.php?id='.$top_cat_value['cat_id'].'';
		$temp_cat_with_child[$top_cat_value['cat_id']]['highlight'] = '0';
		if ($cat_id == $top_cat_value['cat_id']){
			$temp_cat_with_child[$top_cat_value['cat_id']]['hightlight'] = "1";
		}
		foreach ($nontop_cat_arry as $nontop_cat_key => $nontop_cat_value){
			if ($top_cat_value['cat_id'] == $nontop_cat_value['parent_id']){
				$nontop_cat_value['hightlight'] = "0";
				if ($type == 0){$cat_id = $cat_id_of_goods;}
				if ($cat_id == $nontop_cat_value['cat_id']){////hight goods id////
					$nontop_cat_value['hightlight'] = "1";
					$temp_cat_with_child[$nontop_cat_value['parent_id']]['hightlight'] = "1";
					}
					
				$nontop_cat_value['url'] = 'category.php?id='.$nontop_cat_value['cat_id'].'';
				$temp_cat_with_child[$top_cat_value['cat_id']]['children'][] = $nontop_cat_value;
			}
		}
	}
	$cat_arry = $temp_cat_with_child;$temp_cat_with_child = array();
	//showr($cat_arry);
...
function get_navigator($ctype = '', $catlist = array())
{
	...
	/*$string = strtolower($cur_url);
	$split = str_split($string);
					$counter = 0;
					foreach($split as $key => $value){
						$counter = $counter + 1;				
						if(ord($split[$key-2]) == 105 && ord($split[$key-1]) == 100 && ord($value) == 61){
							$postionofidstart = $counter;
							break;
						}
					}
					$cur_url_first_part = substr($cur_url,0,$postionofidstart);					
					
					$cur_id_start = substr($cur_url,$postionofidstart);
					
					$counter = 0;
					$split = str_split($cur_id_start);
					foreach($split as $key => $value){
						$postionofidend = $counter = $counter + 1;
						if (!(ord($value) > 48 && ord($value) < 57) ){
							$postionofidend = $counter;
							break;
						}
					}
					$cur_id= substr($cur_id_start,0,$postionofidend);
					$cur_url_last_part = substr($cur_id_start,$postionofidend);
					$cur_url = $cur_url_first_part . $cur_id . $cur_url_last_part;

		
		$cat_id = get_goods_cat_id($cur_id);

		if (!empty($cat_id)){
			$parent_id = get_goods_parent_id($cat_id);//print ($parent_id);print "<br>";print "<br>";
			if (!empty($parent_id)){
				$cur_url = $cur_url_first_part . $parent_id . $cur_url_last_part;
			}
			else{$cur_url = $cur_url_first_part . $cat_id . $cur_url_last_part;}
		}
		//print ($cur_url);
		//print ($cur_url);*/
    ...
    {	
		...
		//print ($cur_url);print "<br>";
		//print ($v['url']);print "<br>";print "<br>";
