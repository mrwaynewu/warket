...
//-- 系统
/*------------------------------------------------------ */
$file_name = '/flash_data.xml';

$file_name_array = array();
$file_name_array['main']  = '/banner/index_main_banner.xml';
$file_name_array['vege']  = '/banner/index_vege_banner.xml';
$file_name_array['fruit'] = '/banner/index_fruit_banner.xml';
$file_name_array['meat']  = '/banner/index_meat_banner.xml';
$file_name_array['wok']   = '/banner/index_wok_banner.xml';
$file_name_array['re_men_shai_dan']   = '/banner/index_re_men_shai_dan_banner.xml';
$file_name_array['re_men_huo_dong']   = '/banner/index_re_men_huo_dong_banner.xml';

$gotoid_array = array();
$gotoid_array['main'] = 'main_banner_td';
$gotoid_array['vege'] = 'vege_banner_td';
$gotoid_array['fruit'] = 'fruit_banner_td';
$gotoid_array['meat'] = 'meat_banner_td';
$gotoid_array['wok'] = 'wok_banner_td';
$gotoid_array['re_men_shai_dan'] = 're_men_shai_dan_banner_td';
$gotoid_array['re_men_huo_dong'] = 're_men_huo_dong_banner_td';

if (isset($_REQUEST['type']) === false || $_REQUEST['type'] == ''){$_REQUEST['type'] = 'main';}
if (isset($_REQUEST['position']) === false || $_REQUEST['position'] == ''){$_REQUEST['position'] = 'main';}
if (isset($_REQUEST['show_title_field']) === false || $_REQUEST['show_title_field'] == ''){$_REQUEST['show_title_field'] = 0;}
// predefined type are main, vege, fruit, meat, wok
$file_name = $file_name_array[$_REQUEST['type']];

$gotoid = $gotoid_array[$_REQUEST['type']];
...
if ($_REQUEST['act']== 'list')
{
    ...
    $main_banner_db['file']  = get_banner_xml($file_name_array['main']);
    $main_banner_db['name']  = 'main_banner_db';
    $vege_banner_db['file']  = get_banner_xml($file_name_array['vege']);
    $vege_banner_db['name']  = 'vege_banner_db';
    $fruit_banner_db['file'] = get_banner_xml($file_name_array['fruit']);
    $fruit_banner_db['name'] = 'fruit_banner_db';
    $meat_banner_db['file']  = get_banner_xml($file_name_array['meat']);
    $meat_banner_db['name']  = 'meat_banner_db';
    $wok_banner_db['file']   = get_banner_xml($file_name_array['wok']);
    $wok_banner_db['name']   = 'wok_banner_db';
	$re_men_shai_dan_banner_db['file']   = get_banner_xml($file_name_array['re_men_shai_dan']);
    $re_men_shai_dan_banner_db['name']   = 're_men_shai_dan_banner_db';
	$re_men_huo_dong_banner_db['file']   = get_banner_xml($file_name_array['re_men_huo_dong']);
    $re_men_huo_dong_banner_db['name']   = 're_men_huo_dong_banner_db';
    ...
    $banner_db = array(
                        ...
                        $meat_banner_db,
                        $wok_banner_db,
						$re_men_shai_dan_banner_db,
						$re_men_huo_dong_banner_db
                    );

    //print '<pre>';print_r($vege_banner_db);print '</pre>';

    foreach ($banner_db as $key => $val){
        foreach ($val['file'] as $index => $item){
            if (strpos($item['src'], 'http') === false)
            {
                $val['file'][$index]['src'] = $uri . $item['src'];
            }
        }
            $smarty->assign($val['name'], $val['file']);
    }
...
elseif($_REQUEST['act']== 'del')
{
    ...
    $id = (int)$_GET['id'];
    $flashdb = get_banner_xml($file_name);
    if (isset($flashdb[$id]))
    {
        $rt = $flashdb[$id];
    }
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $sort = 0;
        $rt = array('act'=>'add','img_url'=>$url,'img_src'=>$src, 'img_sort'=>$sort, 'type'=>$_REQUEST['type'], 'position'=>$_REQUEST['position'], 'price'=>$_REQUEST['price'], 'title'=>$_REQUEST['title']); 
        $width_height = get_width_height($_REQUEST['banner_width'],$_REQUEST['banner_height']);
        assign_query_info();
        ...
        $smarty->assign('rt', $rt);
		$smarty->assign('gotoid', $gotoid);
        $smarty->assign('ur_here', $_LANG['add_picad']);
		$smarty->assign('show_title_field', $_REQUEST['show_title_field']);
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        // 插入新数据
        array_unshift($flashdb, array('src'=>$src, 'url'=>$_POST['img_url'], 'text'=>$_POST['img_text'] ,'sort'=>$_POST['img_sort'],'position'=>$_POST['position'],'price'=>$_POST['price'],'title'=>$_POST['title']));
        ...
        put_banner_xml($_flashdb,$file_name);
        set_flash_data($_CFG['flash_theme'], $error_msg = '');
        $links[] = array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list#'.$gotoid);
        sys_msg($_LANG['edit_ok'], 0, $links);
    }//print ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;print '<br>';print DATA_DIR . '/afficheimg/' . $name;
}
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $flashdb = get_banner_xml($file_name); //取得数据
    if (isset($flashdb[$id]))
    {
        $rt = $flashdb[$id];
    }
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $flashdb[$id] = array('src'=>$src,'url'=>$_POST['img_url'],'text'=>$_POST['img_text'],'sort'=>$_POST['img_sort'],'position'=>$_POST['position'],'price'=>$_POST['price'],'title'=>$_POST['title']);
...
function get_banner_xml($file_name)
{
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . $file_name))
    {
        ...
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\sposition="([^"]*)"\sprice="([^"]*)"\stitle="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . $file_name), $t, PREG_SET_ORDER))
        {
            preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\sposition="([^"]*)"\sprice="([^"]*)"\stitle="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . $file_name), $t, PREG_SET_ORDER);
        }
//print "<pre>"; print_r($t); print "</pre>";
        if (!empty($t))
        {
            foreach ($t as $key => $val)
            {
                ...
                $flashdb[] = array('src'=>$val[1],'url'=>$val[2],'text'=>$val[3],'sort'=>$val[4],'position'=>$val[5],'price'=>$val[6],'title'=>$val[7]);
            }
        }
    }
    return $flashdb;
...
/*function put_banner_xml($flashdb)
{
    if (!empty($flashdb))
    {
...
function put_banner_xml($flashdb,$file_name)
{
    if (!empty($flashdb))
    {
        ...
        foreach ($flashdb as $key => $val)
        {
            $xml .= '<item item_url="' . $val['src'] . '" link="' . $val['url'] . '" text="' . $val['text'] . '" sort="' . $val['sort'] . '" position="' . $val['position'] . '" price="' . $val['price'] . '" title="' . $val['title'] . '"/>';
        }
        $xml .= '</bcaster>';
...
/*function get_width_height($_REQUEST['banner_width'],$_REQUEST['banner_height'])
{
...
function get_width_height($banner_width,$banner_height)
{
    $curr_template = $GLOBALS['_CFG']['template'];
    $path = ROOT_PATH . 'themes/' . $curr_template . '/library/';
    $template_dir = @opendir($path);
    ...
    $width_height = array();
	$width_height['width'] = $banner_width;
	$width_height['height'] = $banner_height;
...
{
    $flashdata = get_banner_xml($file_name);
    if (empty($flashdata))
    {
        $flashdata[] = array(
